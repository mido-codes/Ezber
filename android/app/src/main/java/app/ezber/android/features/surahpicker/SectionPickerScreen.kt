...
fun SectionPickerScreen(
    ...
    ScreenScaffold(
        title = stringResource(R.string.title_section_picker),
        onBack = onBack,
        actions = {
            TextButton(
                onClick = {
                    if (surah != null) onContinue(surah, VerseRange(start, end))
                },
                enabled = surah != null,
            ) {
                Text("Continue")
        ...
        Column(
            ...
            EzberCard {
                ...
                if (previewVerse != null) {
                    TransliterationText(
                        text = previewVerse.transliteration.ifEmpty { "Transliteration not available yet." },
