...
function SectionPicker() {
  ...
  const { t } = app
  const ready = Boolean(data && range)
  const surah = data?.surah

  return (
    <Screen>
      <AppHeader title={surah?.name_latin ?? t('section.title')} eyebrow={t('section.title')} back />

      {error ? (
        <ErrorBlock
          message={error === 'offline' ? t('player.offline') : t('player.loadFailed')}
          onRetry={reload}
        />
      ) : !ready ? (
        ...
        <>
          {app.content?.isPlaceholderSurah(surahId) ? <PlaceholderReadingNote /> : null}
          <p className="text-sm text-muted-foreground">{t('section.hint')}</p>

          <div className="flex flex-wrap gap-2">
            {[
              {
                label: `1–${Math.min(5, surah!.verses_count)}`,
                start: 1,
                end: Math.min(5, surah!.verses_count),
              },
              {
                label: `1–${Math.min(13, surah!.verses_count)}`,
                start: 1,
                end: Math.min(13, surah!.verses_count),
              },
              ...
              return (
                <button
                  ...
                  onClick={() =>
                    setRange((current) =>
                      current
                        ? { start: current.start, end: Math.max(current.start, ayah.ayah) }
                        : { start: ayah.ayah, end: ayah.ayah },
                    )
                  }
                ...
                >
                  <span className="flex items-center justify-between">
                    <span
                      className={`text-sm font-medium ${isEdge ? 'text-primary' : 'text-foreground'}`}
                    >
                      {ayah.verse_key}
                    </span>
          ...
          <div className="fixed inset-x-0 bottom-0 z-30 mx-auto w-full max-w-2xl px-5 pt-3 pb-[max(1rem,env(safe-area-inset-bottom))]">
            <div className="rounded-3xl border border-border bg-card/95 p-3 shadow-sm backdrop-blur">
              <button
                ...
                onClick={() =>
                  router.push(
                    `/builder/?surah=${surahId}&from=${range!.start}&to=${range!.end}&range=${formatRange(range!.start, range!.end)}`,
                  )
                }
              >
                {t('section.continue')}
