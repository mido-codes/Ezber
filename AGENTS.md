...
- Android build: `cd android && ./gradlew :app:assembleDebug` (JDK 17+, Android SDK platform 36, build-tools 36.0.0). `:app:assembleRelease` and `:app:installDebug` behave as usual. `./gradlew :app:testDebugUnitTest` is the offline JVM suite for the grouped content-bundle parsers and the lazy fetch engine; there is no instrumented test target.
...
- The web/PWA bundle (`content-pipeline/ezber_pipeline/webexport.py`) is a read-only projection of the same canonical artifacts; it must not change the SQLite bundle contract, and every export carries the licence registry, attribution strings and the Tanzil notice. The default export is the grouped lazy layout (`index.json`, `surahs/<id>.json`, `segments/<reciter_id>/<surah_id>.json`); `--layout single` keeps the legacy table files.
...
- Android persistence copies the canonical user DDL into `android/app/src/main/java/app/ezber/android/persistence/UserSchema.kt` and creates `ezber-user.sqlite` from it. When `schema/` changes, update that file and `UserSchema.VERSION` with a row-preserving migration. The content database (`ezber-content.sqlite`, DDL copied into `persistence/ContentSchema.kt`) is an app-owned cache filled lazily from the grouped web bundle: `content/ContentRepository.kt` fetches `/content/index.json` once per process and `/content/surahs/<id>.json` plus `/content/segments/<reciter>/<surah>.json` on demand, verifies the index's sha256 digests, and stores rows against the shared content schema; a changed bundle digest clears only the cache tables. The base URL is a setting with a dev default of `http://10.0.2.2:3000/content/`. The Android audio interfaces (`AudioPlayer`, `AudioSessionService`) are Media3-shaped but do not depend on Media3 yet.
...
- CarPlay lives in `ios/Ezber/CarPlay/`: a preset-list template drives the shared drill engine, and the verse/transliteration lines are mapped into the engine's now-playing description (`CarPlayNowPlayingBuilder`) so the system now-playing screen follows the preset's display options without fighting the engine's own publication. `ios/CARPLAY.md` owns the captain-facing setup: the `com.apple.developer.carplay-audio` entitlement and the scene manifest in `ios/Info.plist` (outside the synchronized `Ezber/` group on purpose).
...
- iOS screens render from placeholder content (`ios/Ezber/Placeholder/PlaceholderContent.swift`) until the content pipeline bundle ships; Android screens read the lazy content cache and gate on `ui/ContentGate.kt` for loading/error/retry states.
