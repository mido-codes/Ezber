...
.PHONY: help pipeline pipeline-offline verify web test clean
...
help:
	@printf '%s\n' \
		...
		'make web               Export the built bundle as a web/PWA payload under build/web/' \
...
web: pipeline
	cd content-pipeline && $(PYTHON) -m ezber_pipeline export-web
...
clean:
	rm -rf content-pipeline/.cache content-pipeline/build/ezber-content.sqlite \
		content-pipeline/build/ezber-content.sqlite-journal content-pipeline/build/web
